        ...
        <View style={styles.pillRow}>
          {preferenceOptions.theme.map((opt) => (
            <TouchableOpacity
              ...
              style={[
                styles.pill,
                preferences.theme === opt.key && styles.pillActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.pillText,
                  preferences.theme === opt.key && styles.pillTextActive,
                ]}
              >
                {opt.label}
              </Text>
      ...
      <View style={styles.sectionRow}>
        ...
        >
          <Text style={styles.langText}>
            {preferenceOptions.language.find(
              (l) => l.key === preferences.language
            )?.label || "Select"}
          </Text>
        </TouchableOpacity>
        <Modal
              ...
              <Switch
                value={
                  preferences.notifications[
                    opt.key as keyof typeof preferences.notifications
                  ]
                }
                onValueChange={() => toggleNotif(opt.key as any)}
                trackColor={{ false: "#e5e7eb", true: "#dbefb5" }}
                thumbColor={
                  preferences.notifications[
                    opt.key as keyof typeof preferences.notifications
                  ]
